...
export type TelemetryTimerMetadata = ActivityTimerMetadata;
...
export function activeElapsedMsAtTimestamp(
  ...
  return movingElapsedMsAtTimestamp(timestampMs, startTimestampMs, intervals);
}
...
export function buildTelemetryPoints(
  ...
  timerMetadata?: TelemetryTimerMetadata | null,
  timeBasis: ActivityTimeBasis = "moving",
): TelemetryPoint[] {
  const intervals = getReliableStoppedIntervals(timerMetadata);
  const useActiveTimeline = mode === "distance" || timeBasis === "moving";

  return records.flatMap((record) => {
    if (useActiveTimeline && intervals.length && isTimestampStopped(record.timestamp_ms, intervals)) return [];

    const distanceMeters = finiteDistanceMeters(record);
    const elapsedMs = Math.max(0, record.timestamp_ms - startTimestampMs);
    const relMs = useActiveTimeline
      ? movingElapsedMsAtTimestamp(record.timestamp_ms, startTimestampMs, intervals)
      : elapsedMs;
...
export function buildTelemetryXAxisBounds(
  points: Array<{ x: number | null }>,
  explicitMax?: number,
): TelemetryXAxisBounds {
  let max = typeof explicitMax === "number" && Number.isFinite(explicitMax)
...
export function buildLapMarkers(
  ...
  return lapTimestampsUtc
    ...
    .map((timestamp, idx) => {
      ...
      const relMs = timeBasis === "total"
        ? Math.max(0, parsed - startTimestampMs)
        : activeElapsedMsAtTimestamp(parsed, startTimestampMs, timerMetadata);
