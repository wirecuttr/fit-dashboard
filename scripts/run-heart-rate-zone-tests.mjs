...
try {
  const compile = spawnSync(tscBin, ["-p", "tsconfig.heart-rate-zone-test.json", "--outDir", outDir], { stdio: "inherit" });
  if (compile.status !== 0) process.exit(compile.status ?? 1);

  writeFileSync(join(outDir, "package.json"), '{"type":"commonjs"}\n');
  const run = spawnSync(process.execPath, [join(outDir, "scripts/test-heart-rate-zones.js")], { stdio: "inherit" });
